...
export function managedReadinessRequest(repo, numbers) {
  ...
  if (process.env.REVIEW_PR_RESULT_PROTOCOL === 'current-review-v1') {
    return { ok: true, action: 'verdict-protocol-no-label', writes: 0 };
  }
...
export async function reconcilePrNumber(prNumber, { dryRun = false } = {}) {
  ...
  // Preserve main's explicit protocol and its default-off configuration semantics.
  if (repo === MIVO_REPO && process.env.REVIEW_PR_RESULT_PROTOCOL === 'current-review-v1') {
