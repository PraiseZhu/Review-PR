...
// 查询,不产生任何写操作)喂给 fetchHeadCheckContexts + classifyRequiredChecks +
// evaluateAuthorizedFastMerge 的完整链路,验证 2026-08-01 owner 裁决(decision 2 收窄
// 阻断面)在真实数据下依然成立,而不只是纯函数层面的构造数据。
//
...
import { fetchHeadCheckContexts, classifyRequiredChecks, evaluateAuthorizedFastMerge, findApproveMergeAuthorization } from '../scripts/lib.mjs';
...
const LIVE_REPO = { owner: 'xindong', repo: 'mivo-canvas', pr: 394 };
...
test('端到端:findApproveMergeAuthorization 对真实 comments 形状(空评论列表)不抛错,fail-closed 返回 null', () => {
  ...
  const r = findApproveMergeAuthorization({ comments: [], admins: ['PraiseZhu'], latestCommitDate: '2026-08-01T00:00:00Z' });
