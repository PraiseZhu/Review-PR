...
> **R8 拆节后锚已迁移（2026-08-10）**：#11 拆为三节（A=lib.mjs 锁原语 /
> B=signoff-hold.mjs / C=signoff-release.mjs）。锁原语（acquireHoldLock /
> releaseHoldLock / tryTakeoverStaleLock / parseStartedAtMs / isLockStale /
> LOCK_STALE_MS）本体自 `scripts/signoff-hold.mjs` 迁入 `scripts/lib.mjs`
> （signoff-hold.mjs 保留 re-export 转发,行为等价）。**涉及锁原语的 seam 锚目标
> 文件改为 `scripts/lib.mjs`**（R3-m1/m2/m5/m6/m7、R4-m1/m5/m6 等）；M-R6-1/2/3 与
> M-R7-1 的 seam（resolveGhCallTimeoutMs / main reconcile 接线 /
> reconcileDuplicateHoldIssues）未搬动,锚仍指向 `scripts/signoff-hold.mjs`。
> 全部条目已在 C 节 tip（`feat/signoff-gates-p1c-signoff-release`）复现仍红,见各条
> 「本轮实测」。复现前提（/tmp 副本 + node --test）不变。
