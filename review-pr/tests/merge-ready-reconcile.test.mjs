...
import { reconcileMergeReady, resolveMergeReadyConfig, managedReadinessRequest, READY_LABEL, MIVO_REPO } from '../scripts/merge-ready-reconcile.mjs';
...
function runCli(args, fixture, protocol = '') {
  return spawnSync(process.execPath, ['scripts/merge-ready-reconcile.mjs', ...args], { cwd: join(dirname(new URL(import.meta.url).pathname), '..'), encoding: 'utf8', env: { ...process.env, REVIEW_PR_RESULT_PROTOCOL: protocol, REVIEW_PR_RULES_FILE: fixture.rulesFile, PATH: `${fixture.root}:${process.env.PATH}` } });
}

test('current-review-v1 preserves main default-off and enabled CLI behavior without gh', () => {
  for (const [rules, action] of [[{}, 'disabled'], [{ mergeReady: { enabled: true, repo: MIVO_REPO } }, 'verdict-protocol-no-label']]) {
    const r = runCli(['7'], cliFixture(rules), 'current-review-v1');
    assert.equal(r.status, 0, r.stderr);
    assert.deepEqual(JSON.parse(r.stdout), { ok: true, action, writes: 0 });
    assert.doesNotMatch(r.stderr, /unexpected gh command/);
  }
});

test('current-review-v1 takes precedence over Mivo ready APIs and workflow requests', async () => {
  const previous = process.env.REVIEW_PR_RESULT_PROTOCOL;
  process.env.REVIEW_PR_RESULT_PROTOCOL = 'current-review-v1';
  try {
    const expected = { ok: true, action: 'verdict-protocol-no-label', writes: 0 };
    const result = await reconcileMergeReady({ pr, config: { enabled: true }, api: { readPullRequest() { throw Error('must not read or write labels'); } } });
    assert.deepEqual(result, expected);
    assert.deepEqual(managedReadinessRequest(MIVO_REPO, [7]), expected);
    assert.equal(managedReadinessRequest('other/repo', [7]), null);
  } finally {
    if (previous === undefined) delete process.env.REVIEW_PR_RESULT_PROTOCOL;
    else process.env.REVIEW_PR_RESULT_PROTOCOL = previous;
  }
});
