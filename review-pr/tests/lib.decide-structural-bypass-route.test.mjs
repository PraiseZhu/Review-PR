...
test('SC0-2:非 admin + 机械前提满足但零 review(#342/#366 型 fail-open 场景)必须 skip', () => {
  const r = decideStructuralBypassRoute({ structuralCanBypass: true, reviewDecision: '', isAdminAuthor: false });
...
test('reviewDecision=null(GraphQL 常见形态)同样必须挡住,不能因为"非 CHANGES_REQUESTED"就误判可合', () => {
  const r = decideStructuralBypassRoute({ structuralCanBypass: true, reviewDecision: null, isAdminAuthor: false });
  assert.equal(r.route, 'skip-structural-block');
});

test('非 admin + 真实 APPROVED review → 照常直接 bypass 合并(不因本次修复被误伤)', () => {
  const r = decideStructuralBypassRoute({ structuralCanBypass: true, reviewDecision: 'APPROVED', isAdminAuthor: false });
  assert.deepEqual(r, { route: 'bypass-structural-block', basis: 'approved' });
});

test('SC0-3:admin(如 ownPr)缺 APPROVED → 进独立审查,不是直接合并、也不是跳过', () => {
  const r = decideStructuralBypassRoute({ structuralCanBypass: true, reviewDecision: '', isAdminAuthor: true });
...
test('admin 身份不豁免机械前提 —— canBypass/allowlist 不满足时仍 skip', () => {
  const r = decideStructuralBypassRoute({ structuralCanBypass: false, reviewDecision: '', isAdminAuthor: true });
  assert.equal(r.route, 'skip-structural-block');
});

test('admin 且已有真实 APPROVED(如被其他人 approve 过)→ 优先走 approved 路径,不必进 admin-trust', () => {
  const r = decideStructuralBypassRoute({ structuralCanBypass: true, reviewDecision: 'APPROVED', isAdminAuthor: true });
  assert.deepEqual(r, { route: 'bypass-structural-block', basis: 'approved' });
});
