...
结尾必须消除"要去求人重审"的心理负担；「建议」明确标注不阻断合并。
...
人**的表达。收件人解析不到（`resolve-author-feishu.mjs` 的 `matched` 为空）时不
猜测、不硬发，按其 `fetchErrors` 是否非空区分"名录没这人"与"名录读不到"，如实
写进汇总。
...
纪律：必须明确"觉得不用改也行"，不预设对方必须服从审查意见；冲突场景把中间
那句换成"和 `<base>` 有冲突，卡着合不了"，收尾换成具体操作（merge 最新
`origin/<base>` 后 push）。
...
纪律：**第一句必须先澄清"不是代码问题"**；全条无傲娇；必须写明放行方式
（讨论 issue 回复 / Approve / 标回 Ready 任一皆可）。
...
纪律：致谢给足；摘要中性；傲娇最多一个词（如"顺手"），公开场合不过量。

### 模板 F：给 owner 的每轮汇总

沿用 6.1 现有的两组格式与"需要你"加粗纪律，**不改结构**，只允许语气略活
（如"今天扫了 6 个，合了 2 个"、"排队 1 个，明天继续"）。6.1 原有的
"禁流程行话、按对照表翻人话、禁原始 JSON"等纪律全部保留，不因语气放开而放松。
