...
description: >
  ...
  飞书通知流程；使用独立审查 agent、前置 gate、GitHub review/merge 和 scheduler pre-run
...
- **输出纪律**：打回评论、内部汇总、飞书通知一律只写文件、行号与类型，绝不引用
...
1. **扫描**：一次运行 `context.mjs --scan-all`，消费 `results` 和
   ...
   **跳过不能对作者静默**：分类完成后，把因作者侧可自解原因被 skip 的候选批量交给
   ...
   同一批 thread／同一 head 只评一次，posted=true 的候选在 6.1 汇总行注明「已提醒作者」。
...
**auto 模式必须把完整摘要主动推送为飞书卡片**：run-log 落盘、自进化复盘完成后，
把 6.1 摘要原文用宿主的飞书 bot 推送能力发给维护者本人——该通道以 markdown 卡片
（interactive/lark_md）渲染，链接可点击，不受 scheduler 通知转发的截断限制；长度上限
以宿主工具声明为准（当前 30000 字符），超限时先压缩行内容再分段发送，不允许砍掉任何
PR 行。scheduler 转发的短通知只当作"本轮已结束"的提示，完整内容以卡片为准。推送能力
不可用或发送失败时不重试轰炸、不影响收尾：保留拟定文案，并在会话末尾摘要里注明
"飞书卡片未送达"。交互模式不主动推送——用户就在会话里，推送等于让人收两份。

**会话的最后一条消息必须是且只能是人类可读摘要**（同一份 6.1 摘要，也是卡片未送达时
的兜底）——scheduler 的桌面/飞书通知会
直接转发会话末尾内容，末尾若是 JSON，用户就会在飞书收到一坨 JSON（这正是历史上
飞书收到 JSON 的事故根因，禁止复发）。发给人看的任何渠道（飞书群、私聊、交互模式的
最终回复）一律禁止原始 JSON，也不贴 run-log 落盘路径。摘要模板（空分组整组省略，
...
**PR 号必须是可点击链接**：发飞书时优先用支持 markdown 的消息形态（富文本 post 的
`a` 元素或互动卡片 lark_md，宿主飞书工具支持哪种用哪种），PR 号渲染成
`[#123](https://github.com/<owner>/<repo>/pull/123)`；发送渠道只支持纯文本时不要发
`[..](..)` 原文——退化为 `#123 https://github.com/<owner>/<repo>/pull/123`
（飞书纯文本会自动把裸 URL 变成可点击链接）。行内引用的讨论 issue（如“issue #88”）
