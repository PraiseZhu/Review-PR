...
- `fix-worktree-cleanup-grace-zero-coerced` **fix-worktree-cleanup --grace-minutes 0 被 || 30 吃掉** — 出现 1 次,首见 2026-09-09,最近 2026-09-09,status: open
  - 现象:交互审合并 PR #529 后按 SKILL/memory 跑 fix-worktree-cleanup.mjs --pr 529 --grace-minutes 0，输出仍 graceMinutes=30，removedWorktrees=[]。根因：Number('0')||30 把合法 0 当成 falsy。审查树是 detached，cleanup-branch 会拒删；本轮改用 git worktree remove --force .worktrees/review-pr/pr-529 收掉。
  - 提案:把 graceMinutes 解析改成 Number.isFinite 判定，允许 0；或文档写明 0 无效必须用 git worktree remove --force。属脚本语义修正，需维护者拍板后改脚本。
