 ...
 * 未命中,退到 SKILL 5.0 的二级语义判断兜底,不在这里加标点归一化(过度归一化标点会
 * 引入假碰撞,把两个真的不同的不变量判成同一个,风险比"退到语义兜底"更大)。
 *
 * ⚠ 2026-08-03 终审 finding 3 更正: 上一版把二级写成「fail-safe:顶多多花一次语义判断,
 * **不会给出错答案**」——**这句是超卖,已删**。二级本来就是 agent 的语义判断,机器只核验
 * 「所引 key 在 state 里确有历史」,**不核验语义对不对**。历史 B 存在时,调用方把一条实际
 * 无关的新 A 错传成 `recurrenceOfKey=keyB`,一级未命中正是允许二级介入的设计态,
 * 代码会接受并记 `matchedBy:'semantic'`——occurrence 就挂错族了。
 * 准确的说法: 二级是**保守方向**的过程约定(判断不了就当新 family,宁可多报一条),
 * 不是「机械上不会出错」的保证。它出错时机器发现不了,只有 review 能发现。
 * 这是 T1 上限,不是缺陷;把它写成「不会给出错答案」才是缺陷。
 *
