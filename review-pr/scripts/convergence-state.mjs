...
// 未观察到中间态重审放大,当时暂不引入 debounce,理由是"cron ~3h 网格本身就是隐式
// debounce"——该前提已于 2026-08-04 随 cron 改为 1h 全天网格而不再成立(观测窗数据
// 仍有效,失效的是那条论证)。1h 网格下连推 commit 被中间态重审的放大风险上升,但仍
// 未实测到,故本轮不动机制、只作废前提:debounce 保留作观测项,重审放大风险待观测。
// 这从来不是"结构上不可能"的全称判断,见 SKILL 5.7 同一处收窄措辞。这次调查顺带查出一个真缺口——非 required 的
